  ...
  return (
    timeList && (
      <Grid item container xs={12} spacing={2}>
        {timeList.map((item) => {
          let shTotal = String(totalweektime(item).h).padStart(2, "0");
          let smTotal = String(totalweektime(item).min).padStart(2, "0");
          return (
            <Grid item container xs={12} key={item.week}>
              <Grid item xs={6}>
                ...
                <Typography variant="p" color="text.secondary">
                  {lang.total_hours}: {shTotal}:{smTotal}
                </Typography>
              </Grid>
              ...
              {item.arr.map((day) => {
                ...
                let smTotal = String(totaldaytime(day).min).padStart(2, "0");
                return (
